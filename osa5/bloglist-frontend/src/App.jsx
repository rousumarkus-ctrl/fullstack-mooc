...
const App = () => {
  ...
  const updateBlog = async (id,blogObject) => {
    ...
    setBlogs(blogs.map(a => a.id === returnedBlog.id ? returnedBlog:a).toSorted((a,b) => b.likes-a.likes))
  }
  ...
  return (
    <div>
      ...
      <h2>blogs</h2>
      {blogs.map(blog =>
        <Blog key={blog.id} blog={blog} ref={blogRef} updateBlog={updateBlog} deleteBlog={deleteBlog} user={user}/>
      )}
    </div>
  )
}
