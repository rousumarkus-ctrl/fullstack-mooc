...
const { multipleBlogs, extraBlog, noLikesBlog, noTitleBlog, noUrlBlog, blogsInDb, usersInDb } = require('./test_helper')
...
describe('initial blogs', () => {
  ...
  describe('blog addition', () => {
    test('a valid blog can be added ', async () => {
      const token = await api.post('/api/login').send({ username:'root',password:'sekret' })
  ...
  describe('blog delete', () => {
    test('succeeds with status 204 if id is valid', async () => {
      ...
      await api.delete(`/api/blogs/${blogToDelete.id}`).set('Authorization',`Bearer ${token.body.token}`).expect(204)
      const blogsAtEnd = await blogsInDb()
  ...
  describe ('blog put', () => {
    test('succeeds', async () => {
      ...
      const blogsAtEnd = await blogsInDb()
      assert.strictEqual(blogsAtEnd.find((blog) => blog.id === blogToUpdate.id).likes,newLikes)
    })
